...
- SQL 명령문 중 SELECT의 처리 속도를 향상 시키기 위해 
  컬럼에 대해서 생성하는 객체
==> 검색하는 경우, 인덱스가 가진 값의 주소값을 가지고 있기 때문에 
    주소값만 찾아서 보는 형태로 속도가 향상됨.
    원래는 전체 테이블을 분석해서 값을 찾아와야 함!
...
-- ** 인덱스를 이용한 검색 방법 **
--> WHERE 절에 인덱스가 지정된 컬럼을 언급하면 됨.

SELECT * FROM EMPLOYEE
WHERE EMP_ID = 215; -- 인덱스 사용 0 (0~1ms)

SELECT * FROM EMPLOYEE 
WHERE EMP_NAME = '대북혼'; -- 인덱스 사용 X (1~ms)
...
SELECT *
FROM TB_IDX_TEST
WHERE TEST_NO = 500000; -- 인덱스 O  1ms 

SELECT *
FROM TB_IDX_TEST
WHERE TEST_ID = 'TEST500000'; -- 인덱스  X 19ms
