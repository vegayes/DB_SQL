...
SELECT DEPT_CODE , SUM(SALARY) 
FROM EMPLOYEE GROUP BY DEPT_CODE ; -- DEPT_CODE가 같은 행끼리 하나의 그룹이 됨.
...
ORDER BY DEPT_CODE , JOB_CODE DESC; -- DEPT_CODE로 그룹을 나누고,
...
-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드
SELECT JOB_CODE 
FROM EMPLOYEE 
GROUP BY JOB_CODE 
HAVING COUNT(*) <= 5;
...
-- (주의 사항! ) 집합 연산자를 사용하기 위한 SELECT문들은
-- 조회하는 컬럼의 타입, 개수가 모두 동일해야 함! 
-- Number타입만, 개수만 같아도됨! 


-- 서로 다른 테이블이지만 컬럼의 타입, 개수만 일치하면 집합연산자 사용가능!
