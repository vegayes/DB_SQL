 ...
 * 
 * - 데이터를 조회(SELECT)하면 조건에 맞는 '행'들이 조회됨. 
 * 이 때, 조회된 행들의 집합을 "RESULT SET" (조회 결과의 집합) 이라고 함. 
 * 
 * - RESULT SET은 0개 이상의 행을 포함할 수 있다. 
 * 왜 0개? ==> 조건에 맞는 행이 없을 수 있다. ( 데이터 오류는 아님! ) 
 * 
 */

-- [작성법]
-- 1) SELECT 컬럼명 FROM 테이블명;
--> 어떤 테이블의 특정 컬럼을 조회하겠다. ( EMP_ID, EMP_NAME 등등 )
...
--------------------------------------------------------------------------------------
/* 2) < 컬럼 값 산술 연산 >
  	컬럼 값 : 테이블 내 한 칸(= cell)에 작성된 값(DATA)
*/  	
...
-- 산술연산은 (내용이) 숫자(NUMBER 타입 , 또는 내용이 숫자인 경우) 가능함! 
...
FROM DUAL 
WHERE 1 = '1'; 
-- DB에서는 숫자 1이랑 문자 1이랑 같다고 생각함. 그러므로 NUMBER 타입이 아니더라도 숫자 연산이 가능함. 
...
-- 2023-08-03 12:05:26.000
-- SYSDATE : 시스템상의 현재 시간(날짜)를 나타내는 상수 (오늘날짜 예약어)

SELECT SYSDATE FROM DUAL; 
-- 4) DUAL(Dumy tAbLe) 테이블 : 가짜 테이블 (임시 조회용 테이블) 
...
--> 6) (필수) DB의 리터럴 표기법은 '' 홑따옴표 임!

SELECT EMP_NAME, SALARY, '원 입니다.' FROM EMPLOYEE; 

----------------------------------------------------------------------------

-- 7) DISTINCT : 조회 시 컬럼에 포함된 중복된 값을 한 번만 표기  ( 한 행에 대한 중복 없앰.)
/* < 주의 사항 >
...
-- EMPLOYEE 테잉블에서 EMAIL의 _앞에 글자가 세글자인 사원만 조회
-- 이름 , 이메일 조회
...
-- ESCAPE
-- ESCAPE문자 뒤에 작성된 _는 일반 문자로 탈출한다는 뜻
-- #, ^사용
--> 사용 후 ESCAPE 문자를 사용했다고 알려주기! 
SELECT  EMP_NAME , EMAIL 
FROM EMPLOYEE
...
AND (DEPT_CODE = 'D9' OR DEPT_CODE  = 'D6')  -- AND가 OR보다 우선순위 높음.
...
OR DEPT_CODE IS NULL ; -- 부서코드 없는 2명 포함  14명
		--   IS NOT NULL ; 도 가능  
		--  이 값은 null이 아닌 값 모든 것을 가져오기 때문에
        --  위의 조건절이 무의미해짐.? 
...
ORDER BY 3 DESC; -- 조회할 컬럼의 3번째 꺼 

-- 입사일 순서대로 이름, 입사일 조회 ( 별칭 사용 )

SELECT EMP_NAME  이름 , HIRE_DATE  입사일
FROM EMPLOYEE 
ORDER BY 입사일 ; -- 별칭사용도 가능!


/* 정렬 중첩 : 대분류 정렬 후 소분류 정렬  (순서가 중요해짐!) */
