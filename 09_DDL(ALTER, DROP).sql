...
------------------------------------------------------------------------
-- Q1) 이미 복사한 값에 중복이 있는경우 UNIQUE의 제약조건을 걸게 되면??
--> 아마도 안 변함.
...
INSERT INTO DEPT_COPY VALUES('D9','총무부','L2');
...
ADD CONSTRAINT DEPT_COPY_TITLE_U 
UNIQUE(DEPT_TITLE);
--> ORA-02299: 제약 (KH.DEPT_COPY_TITLE_U)을 사용 가능하게 할 수 없음 - 중복 키가 있습니다
...
-- Q2) 제약조건명 이름을 가지지 않은 경우 
-->> 제약조건명을 만들지 않으면 내부적으로 미리 만들어 놓음. 
-->> 제약조건명을 찾아서 지워야 함!
...
NOT NULL(DEPT_TITLE);
-- ORA-00904: : 부적합한 식별자
--> NOT NULL의 제약조건은 새로운 제약조건을 추가하는 것이 아닌
-- 컬럼 자체에 NULL값을 허용/비허용 제어하는 성질을 변경의 형태로 인식됨.

--->> 그러므로, MODIFY(수정하다) 구문을 사용해서 NULL제어
ALTER TABLE DEPT_COPY 
...
-- Q3) 그러면,컬럼 추가하면서 바로 제약조건까지 함꼐 추가는 불가?
...
-- 테이블은 최소 1개 이상의 컬럼이 존재해야 되기 때문에 ( ==> TABLE) 
-- 모든 컬럼을 다 삭제할 수는 없다. 
...
-- 2) DDL과 DML 구문을 섞어서 수행하면 안됨.
--> DDL 수행중에 DML 수행을 하면 DML이 COMMIT을 수행하지 않아도, 자동 COMMIT됨.

COMMIT;
SELECT * FROM TB2;

INSERT INTO TB2 VALUES(55,5);
INSERT INTO TB2 VALUES(6,6);

--> 이 상태에서 ROLLBACK하면 뒤로 돌아가짐 (즉, TB2_COL 5,6 값 없어짐)
SELECT * FROM TB2;
...
SELECT * FROM TB2;
--> ROLLBACK 불가. 왜냐하면, DDL을 이용해 컬럼명을 바꾸면서 강제 COMMIT 상태가 되었기 때문에


ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;
CREATE USER kh IDENTIFIED BY kh123;
