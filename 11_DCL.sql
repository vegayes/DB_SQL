...
CREATE USER yes_sample IDENTIFIED BY 1234;

-- 2. 새 연결(접속방법) 추가
--> ORA-01045: 사용자 YES_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, .... TO 사용자명 ;
GRANT CREATE SESSION TO yes_sample;
...
--> CREATE SESSION 권한을 주었기 때문에 테이블 생성 권한은 없음.
-->>>>>> CREATE TABLE : 테이블 생성 권한
-->>>>>> + 데이터를 저장할 수 있는 공간(TABLESPACE) 할당

-- 6. (SYS) 테이블 생성 권한 + TABLESPACE 할당 
GRANT CREATE TABLE TO yes_sample;

ALTER USER yes_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO yes_sample;
...
-- kh / yes_sample 사용자 계정끼리 서로 객체 접근 권한 부여 

-- 1. (sample) kh계정(다른 계정)의 EMPLOYEE 테이블 조회
SELECT *
FROM kh.EMPLOYEE;
...
-- 2.(kh) yes_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO yes_sample;


-- 3. (sample) 다시 kh.EMPLOYEE 조회 ==> 성공! 
SELECT *
FROM kh.EMPLOYEE;
...
REVOKE SELECT ON EMPLOYEE FROM yes_sample;


-- 5. (sample) 다시 kh.EMPLOYEE 조회 ==> 권한 회수되어서 실패!
SELECT *
FROM kh.EMPLOYEE;
...
REVOKE RESOURCE, CONNECT FROM kh;
