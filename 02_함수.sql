...
ROUND( MONTHS_BETWEEN(SYSDATE, HIRE_DATE )) "근무한 개월 수 ",   
...
FROM DUAL;
-- 날짜 형식 안에는 '' 이지만, 문자 넣을려면 "" 넣기! 
-- 년, 월, 일이 날짜를 나타내는 패턴으로 인식이 안되니까 오류 발생.,
...
SELECT TO_DATE('230803 101835') FROM DUAL;
--SQL Error [1861] [22008]: ORA-01861: 리터럴이 형식 문자열과 일치하지 않음
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인지 시켜야 함. 
--> 포맷 쓰기!
...
'RRMMDD'),'YYYY"년" MM"월" DD"일"') 생년월일
FROM EMPLOYEE;   -- DATE의 값이 1962-12-13 일로 받았기 때문에, 날짜패턴으로 변경 됨. 
...
SELECT TO_DATE('500505','YYMMDD') FROM DUAL; -- 2050년도
SELECT TO_DATE('500505','RRMMDD') FROM DUAL; -- 1950년도
SELECT TO_DATE('490505','RRMMDD') FROM DUAL; -- 2049년도
...
-- DECODE(계산식 | 컬럼명 , 조건값 1 , 선택값 1, 조건값 2, 선택값 2, ...., 아무것도 일치하지 않을 때)
...
DECODE(JOB_CODE, 'J7', '20%', 'J6', '15%',
'J5','10%','5%') "인상률",
DECODE(JOB_CODE, 'J7', SALARY*(1+0.2), 'J6', SALARY*(1+0.15),
'J5',SALARY*(1+0.1),SALARY *(1+0.05)) "인상된 급여"
FROM EMPLOYEE ;
...
/* 6) 그룹 함수 */
-- 하나 이상의 행을 그룹으로 묶어 연산하고 총합, 평균 등의 
-- 하나의 결과 행으로 반환하는 함수
...
SELECT SUM(SALARY) "사원들의 급여 합 ", AVG(SALARY) "평균"
FROM EMPLOYEE 
WHERE DEPT_CODE ='D9';
...
SELECT MIN(SALARY) "급여 최소값",  MIN(HIRE_DATE) " 빠른 입사일", MIN(EMAIL) "순서가 빠른 이메일"
FROM EMPLOYEE ;

-- 급여 최대값, 가장 늦은 입사일, 알파벳 순서가 가장 느린 이메일
SELECT MAX(SALARY) "급여 최대값",  MAX(HIRE_DATE) " 늦은 입사일", MAX(EMAIL) "순서가 느린 이메일"
FROM EMPLOYEE ;
...
-- COUNT(컬럼명) : NULL을 제외한 실제 값이 기록된 행의 개수를 리턴

--ex) EMPLOYEE 테이블의 행의 개수
SELECT COUNT(*) FROM EMPLOYEE ;

-- BONUS를 받는 사원의 수 (2가지 방법) 
SELECT COUNT(*) FROM EMPLOYEE 
...
SELECT COUNT(DISTINCT DEPT_CODE) FROM EMPLOYEE  ; -- 6행 (NULL 제거) 
