/* VIEW
	...
	** VIEW 사용 목적 **
	...
	2) 테이블의 진짜 모습을 감출 수 있어 보안상 유리함. --> 보안의 기본적인 것만
	...
	***** VIEW 사용 시 주의사항 *****
    ...
    --                      존재하지 않으면 새로 생성.
    -- 2) FORCE / NOFORCE 옵션
    --      FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성 --> 뷰를 테이블보다 미리 만들때? 
...
-- 사번, 이름, 부서명, 직급명 조회결과를 저장하는 VIEW를 생성
CREATE OR REPLACE VIEW V_EMP 
AS (SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME
	FROM EMPLOYEE
	JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
	JOIN JOB USING(JOB_CODE));
--ORA-01031: 권한이 불충분합니다 (view 할 수 있는 권한을 가져야함!)
